...
Correlation rulesets are resolved by exact immutable name and version. The built-in `correlis-sequence/1` immutable ruleset manifest contains only `COR-SEQ-001`, `correlis-sequence/2` contains `COR-SEQ-001` and `COR-SEQ-002`, and `correlis-sequence/3` contains `COR-SEQ-001`, `COR-SEQ-002`, and `COR-SEQ-003` in evaluation order. Stored configurations never automatically upgrade, and each new ruleset requires a separate correlation projection version and relationship graph version. `COR-SEQ-001` executes as a pure operation over the stored relationship and observation-lineage tables. Historical support is bounded by durable ingest sequence, which ensures prior observed `has_vulnerability` support is strictly before the trigger observation and avoids event-time ordering or aggregate latest-state evidence; future relationship state cannot affect an earlier trigger. Matching exploit attempts persist deterministic `exploited` relationships with rule ID `COR-SEQ-001`, rule version `1`, deterministic provenance, and confidence `0.85`. Version 2 extends that chain with deterministic `compromised` relationships from later suspicious process activity after a prior exploit against the same target. Version 3 extends it again with deterministic `moved_laterally_to` relationships from authentication by a previously compromised source. `COR-SEQ-002` and `COR-SEQ-003` use prior observed or deterministic relationship lineage of the required support type and strict ingest-sequence cutoffs rather than event time or aggregate latest state.
...
## COR-SEQ-003 execution

The correlation package includes a pure evaluator for `COR-SEQ-003`, which correlates authentication from a previously compromised source entity to another entity and derives a `moved_laterally_to` candidate. Its historical support lookup is bounded by durable ingest sequence lineage: only observed or deterministic `compromised` relationships with relationship-observation sequences strictly earlier than the authentication trigger can support the candidate. Future relationship evidence is excluded from historical evaluation.

The evaluator is intentionally side-effect free and returns an immutable candidate or `None`. `correlis-sequence/3` registers it in fixed durable dispatch; the correlation projector then persists valid candidates and lineage atomically. Active ruleset versions 1 and 2 remain unchanged and continue to execute only their original rules.
