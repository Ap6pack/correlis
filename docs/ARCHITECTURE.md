...
The built-in immutable ruleset manifest is stored with the configuration and currently contains only `COR-SEQ-001`, `Exploit against known vulnerability`. This PR defines the manifest and hash only; the rule is not executed. There is no correlation handler, no run command, no deterministic relationship write path, no background worker, no public API, no dynamic rules, and no AI dependency.
