...
The evaluator remains intentionally side-effect free and returns an immutable candidate or `None`. Immutable `correlis-sequence/3` activates it through fixed dispatch in the existing generic projector, which atomically persists relationship, evidence, and derivation lineage. The immutable rule manifests are `correlis-sequence/1 = COR-SEQ-001`, `correlis-sequence/2 = COR-SEQ-001 + COR-SEQ-002`, and `correlis-sequence/3 = COR-SEQ-001 + COR-SEQ-002 + COR-SEQ-003`; existing projections never silently upgrade. Historical support is sequence bounded, every derived relationship retains evidence and derivation lineage, and incident and Attack Scene projection remain future work.
