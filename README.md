...
`COR-SEQ-003` performs no writes and is not part of active ruleset version 2. Active rulesets remain unchanged: version 1 contains `COR-SEQ-001`, and version 2 contains exactly `COR-SEQ-001` and `COR-SEQ-002`. Ruleset version 3 and durable projector execution for `COR-SEQ-003` are planned for the next PR.
