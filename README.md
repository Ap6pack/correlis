...
`correlis-store` now includes a pure `COR-SEQ-003` evaluator for authentication from a previously compromised source entity to another entity. The evaluator derives an in-memory `moved_laterally_to` candidate only when historical `compromised` support existed before the authentication trigger, using durable ingest sequence lineage rather than event time.

`COR-SEQ-003` remains a pure evaluator; ruleset version 3 activates it through the existing generic durable projector. The immutable rulesets are `correlis-sequence/1 = COR-SEQ-001`, `correlis-sequence/2 = COR-SEQ-001 + COR-SEQ-002`, and `correlis-sequence/3 = COR-SEQ-001 + COR-SEQ-002 + COR-SEQ-003`. Existing projections never silently upgrade. Historical support is strictly sequence bounded, and every derived relationship retains evidence and derivation lineage. Incident and Attack Scene projection remain future work.
